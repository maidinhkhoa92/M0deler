...
const Histograms: React.FC<Props> = ({className}) => {
  ...
  const xData = useMemo(() => map(data || [], (el) => String(el.Wallet)), [data])
  const yData = useMemo(() => map(data || [], (el) => el['Curr. Balance']), [data])
    ...
    <div className={`card ${className}`}>
      <div className='card-header border-0 pt-5'>
        <h3 className='card-title align-items-start flex-column'>
          <span className='card-label fw-bold fs-3 mb-1'>Histogram of Ending Account Balances Data comes from Balances API endpoint</span>
        </h3>
      </div>
